...
{
	if (otclientV8) {
		// Due to limited packet size (65KB), whole map description may be too big for one packet
		// To fix that problem, every floor will be send separately
		int32_t startz, endz, zstep;
		if (pos.z > 7) {
			startz = pos.z - 2;
...
void ProtocolGame::sendMoveCreature(const Creature* creature, const Position& newPos, int32_t newStackPos, const Position& oldPos, int32_t oldStackPos, bool teleport)
{
	if (creature == player) {
		...
		} else if (teleport || (otclientV8 && newPos.z < oldPos.z && newPos.z == 7)) { // otcv8 fix for too big packet
			NetworkMessage msg;
...
void ProtocolGame::MoveUpCreature(NetworkMessage& msg, const Creature* creature, const Position& newPos, const Position& oldPos)
{
	...
	if (newPos.z == 7) {
		int32_t skip = -1;
		GetFloorDescription(msg, oldPos.x - awareRange.left(), oldPos.y - awareRange.top(), 5, awareRange.horizontal(), awareRange.vertical(), 3, skip);
